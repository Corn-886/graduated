...
public class selectall {
    ...
    public static void main(String[] args) throws SQLException, InterruptedException {
        System.out.println("1开始"+new SimpleDateFormat("ssSSS") .format(new Date() ));
        sql = "select * from feedback_operation_log";//SQL语句
        ...
        sql = "select * from feedback_alarm_config";//SQL语句
        db1 = new DBHelper(sql);//创建DBHelper对象
            ret = db1.pst.executeQuery();//执行语句，得到结果集
        ...
        sql = "select * from at_km_cagegory";//SQL语句
        db1 = new DBHelper(sql);//创建DBHelper对象
        ret = db1.pst.executeQuery();//执行语句，得到结果集
